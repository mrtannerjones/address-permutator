...
exports.main = main
function main(addressString){
	const wordCombos = getAbbreviationCombos(addressString)
    return createPermutations(wordCombos)
}
...
function createPermutations(arr) {  	
	...
	let result = [];
   	let allCasesOfRest = createPermutations(arr.slice(1));
...
exports.getAbbreviationCombos = getAbbreviationCombos
function getAbbreviationCombos(addressString){
	return addressString.split(' ')   
	.map(word => {
       	const abbreviation = getAbbreviation(word)
        return abbreviation === word ? [word] : [word, abbreviation]
	})
}
